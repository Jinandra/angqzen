...
(function() {
  ...
  var BinaryFile = function(strData, iDataOffset, iDataLength) {
      ...
      this.fromBase64 = function(strBase64) {
          data = window.atob(strBase64.replace(/\s/g,''));
      }
  };
  /*
   * DEPENDENCY
  ...
  angular.module('ImageCropper',[])
    .directive('imageCrop', function() {
      ...
      return {
        template: '<div id="image-crop-{{ rand }}" class="ng-image-crop ng-image-crop--{{ shape }}" ng-style="moduleStyles"><section ng-style="sectionStyles" ng-show="step==1"></section><section ng-style="sectionStyles" ng-show="step==2"><canvas class="cropping-canvas" width="{{ canvasWidth }}" height="{{ canvasHeight }}" ng-mousemove="onCanvasMouseMove($event)" ng-mousedown="onCanvasMouseDown($event)"></canvas><div ng-style="croppingGuideStyles" class="cropping-guide"></div>' +
        '<div style="display: none" class="zoom-handle" ng-mousemove="onHandleMouseMove($event)" ng-mousedown="onHandleMouseDown($event)" ng-mouseup="onHandleMouseUp($event)"><span>&larr; zoom &rarr;</span></div>' +
        ' <div class="photo-zoom-btn-box">'+
          '<button class="btn-white-circle" ng-click="zoomOut()"><i class="fa fa-search-minus"></i></button>'+
          '<button class="btn-white-circle" ng-click="zoomIn()" ng-click=""><i class="fa fa-search-plus"></i></button>'+
          '</div>'+
        '</section><section ng-style="sectionStyles" class="image-crop-section-final" ng-show="step==3"><img class="image-crop-final" ng-src="{{ croppedDataUri }}" /></section></div>',
        replace: true,
        restrict: 'AE',
        scope: {
		  crop: '=',
        ...
        link: function (scope, element, attributes) {
            //zoom In custom button
            scope.zoomIn = function(){
               zoomImage(0.05)
            };
            //zoom Out custom button
          ...
          scope.croppingGuideStyles = {
            ...
            top: (scope.width/4)+'px',
            left: (scope.height/4)+'px'
          };*/
		  scope.moduleStyles = {
            width: '100%',
            height: '100%'
        ...
        scope.croppingGuideStyles = {
		  ...
		  function loadImage(base64ImageSrc) {
			...
			//get the EXIF information from the image
              base64ImageSrc = base64ImageSrc.replace(/\s/g,'');
            var byteString = window.atob(base64ImageSrc.split(',')[1]);
          ...
          $finalImg.onload = function() {
            ...
            tempCanvas.width = $canvas.width*0.74 ;
            tempCanvas.height = $canvas.height*0.74;

           tempCanvas.style.display = 'none';

            var tempCanvasContext = tempCanvas.getContext('2d');
            tempCanvasContext.drawImage($finalImg, -($canvas.width*0.13), -($canvas.height*0.13));
